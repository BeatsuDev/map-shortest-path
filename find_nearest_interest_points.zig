...
pub fn find_nearest_interest_points(allocator: std.mem.Allocator, start: *Node, interest_type: u8, node_count: usize, all_interest_points: []InterestPoint, comptime interest_point_count: usize) ![]*InterestPoint {
    var interest_points: [interest_point_count]*InterestPoint = undefined;
    ...
    var found_interest_points: usize = 0;

    while (search_queue.removeOrNull()) |priority_node| {
        for (all_interest_points) |*interest_point| {
            if (interest_point.node_id == priority_node.node.id and interest_point.interest_type & interest_type == interest_type) {
                interest_points[found_interest_points] = interest_point;
                found_interest_points += 1;
            }
            if (found_interest_points >= interest_point_count) break;
        }
        if (found_interest_points >= interest_point_count) break;
